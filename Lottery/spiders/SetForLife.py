...
class SetforlifeSpider(scrapy.Spider):
    name = 'SetForLife'
    allowed_domains = ['www.results.co.uk']
    start_urls = ['http://www.results.co.uk/set-for-life-results']
    custom_settings = {
        # 'USER_AGENT': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/61.0.3163.100 Safari/537.36',
        'ITEM_PIPELINES': { 'Lottery.pipelines.Life': 300 }
    }
            ...
            if (not (row.css('.dateHeader') or row.css('.compHeader'))):
                ...
                for i, cell in enumerate(columns):
                    ...
                    if(i==2):
                        ...
                        prize_text = prize_row[1:].strip()
                        prize.append(prize_text.split()[0])
                    if(i==3):
                        total_row = cell.css('td::text')[0].extract()
                        total_text = total_row[1:].strip()
                        total.append(total_text.split()[0].strip())
